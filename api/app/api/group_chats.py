...
def get_group_chat_messages(url_params, group_chat_id):
    ...
    try:
        results = database_repository.get_group_chat_messages(community_id, group_chat_id, per_page, cursor=cursor)
...
def get_group_chat_message(group_chat_id, message_id):
    ...
    if not member:
        return {"error": "User is not a member of this group chat"}, HTTPStatus.UNAUTHORIZED
    chat_message = database_repository.get_chat_message(group_chat_id, message_id, MessageType.GROUP_CHAT)
    if not chat_message:
...
def join_group_chat(group_chat_id, user_id):
    ...
    try:
        database_repository.add_group_chat_member(
...
def leave_group_chat(group_chat_id, user_id):
    ...
    try:
        database_repository.remove_group_chat_member(group_chat_id, g.current_user.id)
    except NotFoundException as err:
        return {"error": str(err)}, HTTPStatus.NOT_FOUND
